...
function sendToNearestContacts (user, nearestContacts) {

	if ( !nearestContacts || nearestContacts.length == 0 ) {
		logger.info('There are not near contacts');
		return;
	}

	logger.info('Near contacts=' + nearestContacts.length);

	var registrationIds = [], gcmId;
	for ( var f = 0, F = nearestContacts.length; f < F; f++ ) {
		gcmId = nearestContacts[f].gcmId;
		if ( gcmId ) {
